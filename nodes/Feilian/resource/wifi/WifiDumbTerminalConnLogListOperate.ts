...
export default {
	name: '获取哑终端连接日志',
	value: 'wifi:dumb_terminal:conn:log:list',
	order: 40,
	options: [
		{
			displayName: '哑终端 ID',
			name: 'dumb_terminal_id',
			type: 'number',
			default: 0,
			description: '0 表示不传',
		},
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject[]> {
		const dumb_terminal_id = this.getNodeParameter('dumb_terminal_id', index, 0) as number;
		...
		const buildBaseQs = (): IDataObject => {
			...
			if (dumb_terminal_id) qs.dumb_terminal_id = dumb_terminal_id;
		...
		const fetchPage = async (offset: number, pageSize: number) => {
			...
			const requestOptions: IHttpRequestOptions = {
				method: 'GET',
				url: '/api/open/v1/wifi/dumb_terminal/conn/log/list',
				qs,
			};
